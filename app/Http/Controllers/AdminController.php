...
class AdminController extends Controller
{
    ...
    public function show_dashboard(){
        ...
        $total_revenue = Bill::where('Status',[2])->sum('TotalBill');
        $total_sell = BillInfo::join('bill','bill.idBill','=','billinfo.idBill')->where('Status',[2])->sum('QuantityBuy');
    ...
    public function submit_edit_adprofile(Request $request){
        ...
        if ($request->file('Avatar')){
            ...
            $get_image->storeAs('public/admin/images/user',$new_image);
            $admin->Avatar = $new_image; 
            Session::put('Avatar', $new_image);

            $get_old_img = Admin::where('idAdmin', Session::get('idAdmin'))->first();
            Storage::delete('public/admin/images/user/'.$get_old_img->Avatar);
        }
                    ...
                    <div class="card card-block card-stretch mb-0">
                        <div class="card-body">
                            <div class="bg-warning-light rounded">
                                <img src="public/storage/admin/images/product/'.$image.'" class="style-img img-fluid m-auto p-3" alt="image">
                            </div>
